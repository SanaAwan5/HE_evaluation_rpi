...
publickey,privatekey = KeyPair().generate(n_length=1024)
...
class Model():
    ...
    def generate_gradient(self, owner, input, target):
        ...
        grad_values = grad_values.encrypt(publickey)
        print("type type type",type(grad_values))
    ...
    def evaluate_gradient(self, addr, gradient, prikey, pubkey, inputs,
        ...
        candidate.weights = candidate.weights.encrypt(publickey)
        candidate.weights -= gradient.grad_values * alpha
        candidate.weights = candidate.weights.decrypt(privatekey)
