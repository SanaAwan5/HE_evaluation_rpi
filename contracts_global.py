...
class Model():
    ...
    def evaluate_gradient(self, addr, gradient, prikey, pubkey, inputs,targets, alpha=1):
        ...
        #print('length',self.gradient_list[0].grad_values)
        length=len(gradient_list)
        gradient_list[0].grad_values = gradient_list[0].grad_values * alpha
        for i in range (1,length):
            gradient_list[0].grad_values += gradient_list[i].grad_values * alpha    
        avg_grad = gradient_list[0].grad_values / length
        candidate.weights -= avg_grad
        ...
        tx.evalGradient(gradient.id, new_model_error, IPFSAddress().to_ethereum(ipfs_address))
    ...
    def str(self):
        ...
        #s += "Num Grads in Grad List:" + str(len(self.gradient_list)) + "\n"
        return s
